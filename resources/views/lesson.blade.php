...
@section('content')
        ...
        <div class="detail-container grid" style="row-gap: 5rem">
            <div class="watch-data-left" id="watch-data-left">
                ...
                @if ($purchased_course)
                    ...
                    @endif
                @endif
                <div class="swiper-button-next watch-next-icon" id="fullscreen"
                ...
                <ul style="padding-bottom: 5rem;">
                    @foreach ($lesson->course->publishedLessons as $publishedLesson)
                        @if ($publishedLesson->free_lesson)
                            <li>
                                <a
                                    href="{{ route('lessons.show', [$publishedLesson->course_id, $publishedLesson->slug]) }}"><i
                                        class="bx bx-play-circle"></i>{{ $publishedLesson->title }}</a>
                            </li>
                        @else
                            @if ($purchased_course)
                                <li>
                ...
                @if (!$purchased_course)
                        ...
                        @if ($lesson->course->students()->where('user_id', auth()->id())->count() == 0)
                            <form action="{{ route('courses.payment') }}" style="margin-top: 1rem;" method="POST">
                                ...
                                <input type="hidden" name="lesson_id"
                                    value="{{ $lesson->course->publishedLessons[0]->slug }}" />
                    ...
                    @else
                        <a href="{{ route('register') }}?redirect_url={{ route('courses.show', [$lesson->course->slug]) }}"
                            class="button detail-button" style="text-align: center;">Purchase Course
                            (${{ $lesson->course->price }})</a>
                    @endif
                @endif
            </div>
        </div>
    ...
    @if ($purchased_course)
        <section class="section rating">
            ...
            column-gap: 1rem;
            "
                    action="{{ route('courses.rating', [$lesson->course->id]) }}" method="post" method="post">
                    @csrf
                    <select name="rating"
                        style="height: 30px;
                                min-width: 30%;
                                ...
                                outline: none;">
                        <option value="1">1 - burik</option>
                        <option value="2">2 - ngak bagus</option>
                        <option value="3">3 - b aja</option>
                        <option value="4" selected>4 - lumayan</option>
                        <option value="5">5 - terbaiklah</option>
                    </select>
