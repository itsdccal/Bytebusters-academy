            ...
            <div class="detail-data-right">
                        ...
                        <li>
                            @if ($lesson->free_lesson)
                                <a class="lesson-title"
                                    href="{{ route('lessons.show', [$lesson->course_id, $lesson->slug]) }}"><i
                                        class="bx bx-play-circle"></i>{{ $lesson->title }}</a>
                            @else
                                @if (!$purchased_course)
                                    <a class="lesson-title" aria-disabled="false" style="cursor:  alias" href="#"><i
                                            class='bx bx-lock'></i>Another course {{ $lesson->count() }}</a>
                                @else
                                    <a class="lesson-title"
                                        href="{{ route('lessons.show', [$lesson->course_id, $lesson->slug]) }}"><i
                                            class="bx bx-play-circle"></i>{{ $lesson->title }}</a>
                ...
                @else
                    <a href="{{ route('register') }}?redirect_url={{ route('courses.show', [$course->slug]) }}"
                        class="button detail-button" style="text-align: center;">Buy course (Rp{{ $course->price }})</a>
                @endif
            </div>
        </div>
    </section>
